...
- **Зацепка «Помощник Доктора» → действие:** разговор → **результат:** Доктор у себя дома в таверне → **следующая
...
- **Помощник Доктора (doctor_assistant)** — встречает игроков в морге; не связан с убийством в таверне напрямую. См.
  03-npcs.md.
...
- **Критическая зацепка:** «Помощник Доктора говорит, что Доктор в таверне» — ключ к развязке; нельзя терять из-за
  одного провала (Мастер повторяет указание на таверну через другого NPC/стражу).
- **Реакция при допросе:** Помощник Доктора — по уровням L1–L4 см. `characters/doctor_assistant.speech_profile`.
